...
`toad` 프로젝트의 확정된 누적 기록 저장장치다.
작업 시작은 `handoff.md`를 먼저 보고, 현재 상태는 `handoff.cache.md`를 본다.
...
- 대화 중 백엔드가 맡아야 할 기능이 새로 나오면 먼저 `handoff.cache.md`에 적는다.
- 검증/푸시가 끝난 확정 내용만 이 문서에 옮긴다.
- 프론트에 더미로 만든 백엔드 데이터는 `handoff.cache.md`에 먼저 표시하고, 안정화 후 이 문서에 요약한다.
- 실제 백엔드 구축 시 지우거나 API로 교체해야 할 프론트 상태/더미 데이터도 같은 순서로 기록한다.
...
- `sender_key = toad-demo-system` 또는 `채팅 안내`/`테스트 안내`/`연락처 요청` 메시지는 일반 말풍선이 아니라 공지 카드로 표시한다.
- 공지 카드는 두꺼비 mood를 일반 채팅과 다르게 쓰고, 연락처 요청은 금색 계열로 구분하되 두꺼비 이미지는 표시하지 않는다.
- 채팅방 안 상대 두꺼비 사진을 눌러도 상대 프로필 팝업을 연다.
- 상대 프로필 팝업은 화면 중앙의 작은 카드형 모달로 표시한다.
...
- `src/components/Onboarding.jsx`의 로그인 화면에서 `중개사로 시작하기` 버튼을 제거했다.
- 중개사 권한 테스트는 설정의 테스트 아이디 변경으로 한다.
...
- 먼저 `AGENTS.md`, `handoff.md`, `handoff.cache.md`, `src/App.jsx`를 읽고 이어서 작업한다.
- 최근 핵심 수정 파일:
...
- 프론트 임시 백엔드 로직을 추가하면 먼저 `handoff.cache.md`에 기록한다.
