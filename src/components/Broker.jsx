...
export function Broker({ properties = PROPERTIES, brokerProposals = [], preset = {}, menuMode = "all", role, availableRoles, tier = "골드", onSwitchRole, onOpenChat, onRecordProposal, openModal }) {
  ...
  const [mapAddressFilterEnabled, setMapAddressFilterEnabled] = useState(false);
  ...
  useEffect(() => {
    ...
    setMapAddressFilterEnabled(false);
  }, [menuMode]);
  ...
  const applyFilters = () => {
    ...
    setActiveId(null);
    setActiveMapGroup(null);
    setMapAddressFilterEnabled(true);
  };
  ...
  return (
    <div style={{ paddingBottom: 132, background: viewTone.page, minHeight: "100%", position: "relative" }}>
      ...
      <div style={{ padding: "14px 16px 0" }}>
        ...
        {(!isViewedMenu || filterOpen) && <MiniMap items={mapAddressFilterEnabled ? list : mapList} activeId={activePin?.key} onPick={pickPin} tone="green"/>}
        {isViewedMenu && (
